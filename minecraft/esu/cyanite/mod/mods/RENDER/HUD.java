...
import esu.cyanite.events.EventBlock;
...
import esu.cyanite.mod.mods.WORLD.Scaffold;
...
public class HUD extends Mod {
    ...
    private void renderArray(final UnicodeFontRenderer unicodeFontRender, final ScaledResolution scaledResolution) {
        ...
        ArrayList<Mod> mods1 = new ArrayList<Mod>();
        ArrayList<Mod> mods = new ArrayList<Mod>(ModManager.getEnabledModListHUD());
        mods.sort(Comparator.comparingDouble(m -> -unicodeFontRender.getStringWidth(m.getRenderName())));

        for (Mod m : ModManager.getModList()) {
            if (m.isEnabled() || m.getAnimy() != 0) {
                mods1.add(m);
            }
        }

        mods1.sort(Comparator.comparingDouble(m -> -unicodeFontRender.getStringWidth(m.getRenderName())));

        for (final Mod mod : mods1) {
            if (mod.isEnabled()) {
                mod.setAnimy(RenderUtil.toanim(mod.getAnimy(), 12, 12f, 1f));
            } else {
                mod.setAnimy(RenderUtil.toanim(mod.getAnimy(), 0, 12f, 1f));
            }
            ...
            Gui.drawModalRectWithCustomSizedTexture(scaledResolution.getScaledWidth() - n4 - n3 - 14.0f, n2 - 2.5f - 1.5f - 1.5f - 1.5f - 6.0f - 1.0f, 0.0f, 0.0f, unicodeFontRender.getStringWidth(renderName) * 1 + 20 + mod.getAnimy(), 38.5, unicodeFontRender.getStringWidth(renderName) * 1 + 20 + 10, 38.5);
            GlStateManager.disableBlend();
            GlStateManager.enableAlpha();
            GL11.glPopMatrix();
            n2 += unicodeFontRender.FONT_HEIGHT + 1;
        }
        for (final Mod mod2 : mods1) {
            final String renderName2 = mod2.getRenderName();
            unicodeFontRender.drawStringWithShadowWithAlpha(renderName2, scaledResolution.getScaledWidth() - (float) unicodeFontRender.getStringWidth(renderName2) - n3, (float) (n + 1 + mod2.getAnimy() / 2), Colors.WHITE.c, (mod2.getAnimy() / 12f) * 0.8f + 0.05f);
